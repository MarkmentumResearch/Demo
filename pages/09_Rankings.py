...
if show_cur_ticker_hm:
        ...
        alt.Chart(tm)
          ...
          .encode(
              ...
              color=alt.Color("score:Q",
                              ...
                              legend=alt.Legend(orient="bottom", title="Score", gradientLength=140,labelPadding=8)),
              tooltip=[
    ...
    c_blank, mid2, c_blank = st.columns([2.50,3,.3])
    with mid2:
        st.altair_chart(cur_ticker_heat, use_container_width=False)
    st.markdown("<div style='height: 10px;'></div>", unsafe_allow_html=True)
...
x48 = alt.X("model_score:Q", title="Score", scale=alt.Scale(domain=ms_dom))
...
neg48  = base48.transform_filter("datum.model_score < 0").mark_text(align="right", baseline="middle", dx=-10)\
                 .encode(text=alt.Text("model_score:Q", format=",.0f"))
chart48 = (bars48 + pos48 + neg48).properties(title="Score", height=chart_h).configure_title(anchor="middle").configure_axisY(labelAlign="right", labelPadding=1)
