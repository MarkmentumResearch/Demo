...
def _ph_interp_color(val: float, vmax: float):
    """
    Light diverging fill: negative = light red, positive = light green, 0 = white.
    vmax is per-column scaling.
    """
    from reportlab.lib import colors
    ...
    if vmax is None or vmax == 0 or pd.isna(v):
        return colors.white

    # clamp -1..1
    t = max(-1.0, min(1.0, v / float(vmax)))

    # base endpoints (very light)
    ...
    if t < 0:
        a = abs(t)
        r = white[0] * (1 - a) + neg[0] * a
        g = white[1] * (1 - a) + neg[1] * a
        b = white[2] * (1 - a) + neg[2] * a
    else:
        a = t
        r = white[0] * (1 - a) + pos[0] * a
        g = white[1] * (1 - a) + pos[1] * a
        b = white[2] * (1 - a) + pos[2] * a
