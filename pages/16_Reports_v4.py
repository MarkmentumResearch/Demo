...
if include_morning_compass:
    ...
    # Morning Compass preview (existing behavior)
    if include_morning_compass:
        mc_parts = []
        for k in tf_keys:
            asof_k = _asof_date_from_main(k)
            mc_parts.append(f"{k}: {asof_k if asof_k else '(date not found)'}")
        preview_parts.append("Morning Compass – " + " | ".join(mc_parts))

    # Market Overview preview (UI only for now)
    if include_market_overview:
        preview_parts.append(
            "Market Overview – " + ", ".join(market_tf_keys if market_tf_keys else ["(no timeframe selected)"])
        )
