...
# -------------------------
# Research Pack Presets
# -------------------------
PRESETS = {
    "Daily Research Pack": {
        "include_morning_compass": True,
        "include_market_overview": True,
        "include_performance_heatmap": False,
        "include_sharpe_heatmap": False,
        "include_markmentum_heatmap": False,
        "include_directional_trends": False,

        # Morning Compass options
        "mc_timeframes": [],
        "mc_macro": True,
        "mc_correlations": True,
        "mc_top_pct": True,
        "mc_top_mm": True,
        "mc_top_delta": True,
    ...
    "Full Research Pack": {
        ...
        "include_sharpe_heatmap": True,
        "include_markmentum_heatmap": True,
        "include_directional_trends": True,

        "mc_timeframes": ["Weekly", "Monthly"],
        "mc_macro": True,
        "mc_correlations": True,
        "mc_top_pct": True,
        "mc_top_mm": True,
        ...
        "mo_timeframes": ["Weekly", "Monthly", "Quarterly"],
        "mo_top_cards": True,
        "mo_mm_cards": True,
        "mo_daily_extras": True,
        "mo_market_read": True,
    ...
    "Risk & Regime Pack": {
        "include_morning_compass": True,
        "include_market_overview": False,
        "include_performance_heatmap": False,
        "include_sharpe_heatmap": True,
        "include_markmentum_heatmap": False,
        "include_directional_trends": True,

        "mc_timeframes": ["Daily", "Weekly", "Monthly"],
        "mc_macro": True,
        "mc_correlations": True,
        "mc_top_pct": False,
        "mc_top_mm": False,
...
class MorningCompassModule(ReportModuleBase):
    ...
    def ui(self) -> dict:
        ...
        with right:
            ...
            include_pct = st.checkbox(
                "Top Five Leaders/Laggards by % Change",
                value=True,
                key="mc_top_pct"
            )
            include_mm = st.checkbox(
                "Top Five Leaders/Laggards by MM Score",
                value=True,
                key="mc_top_mm"
            )
            include_delta = st.checkbox(
                "Top Five Leaders/Laggards by MM Score Change",
                value=True,
                key="mc_top_delta"
            )
...
def apply_preset(preset_name: str):
    ...
    # Main section checkboxes (these MUST match your checkbox keys)
    st.session_state.select_morning_compass      = p.get("include_morning_compass", False)
    st.session_state.select_market_overview      = p.get("include_market_overview", False)
    st.session_state.select_performance_heatmap  = p.get("include_performance_heatmap", False)
    st.session_state.select_sharpe_heatmap       = p.get("include_sharpe_heatmap", False)
    st.session_state.select_markmentum_heatmap   = p.get("include_markmentum_heatmap", False)
    st.session_state.select_directional_trends   = p.get("include_directional_trends", False)

    # Morning Compass options
    st.session_state.mc_timeframes    = p.get("mc_timeframes", [])
    st.session_state.mc_macro        = p.get("mc_macro", True)
    st.session_state.mc_correlations = p.get("mc_correlations", True)
    st.session_state.mc_top_pct      = p.get("mc_top_pct", True)
    st.session_state.mc_top_mm       = p.get("mc_top_mm", True)
    st.session_state.mc_top_delta    = p.get("mc_top_delta", True)

    # Market Overview options
    st.session_state.mo_timeframes    = p.get("mo_timeframes", [])
    st.session_state.mo_top_cards     = p.get("mo_top_cards", True)
    st.session_state.mo_mm_cards      = p.get("mo_mm_cards", True)
    st.session_state.mo_daily_extras  = p.get("mo_daily_extras", True)
