...
class MarketOverviewModule(ReportModuleBase):
    ...
    def ui(self) -> dict:
        ...
        dfs = mo_load_for_timeframe(tf_key)
        asof = _mo_asof_from_df(dfs[0])
        st.markdown(f"**Preview:** {tf_key} Market Overview – {asof if asof else '(date not found)'}")

        return {
            "tf_key": tf_key,
            "include_top_cards": include_top_cards,
            "include_score_change_cards": include_score_change_cards,
            "include_daily_extras": include_daily_extras if tf_key == "Daily" else False,
            "include_market_read": include_market_read,
        }

    def build(self, options: dict) -> tuple[list[bytes], str]:
        tf_key = options.get("tf_key", "Daily")
        blob = build_market_overview_pdf(
            tf_key=tf_key,
            ...
            include_daily_extras=options.get("include_daily_extras", (tf_key == "Daily")),
            include_market_read=options.get("include_market_read", True),
        )

        # filename stub (packet naming still handled by main download filename)
        asof = _mo_asof_from_df(mo_load_for_timeframe(tf_key)[0])
        date_slug = asof.replace("/", "-") if asof else "report"
        stub = f"market_overview_{tf_key.lower()}_{date_slug}"
        return ([blob], stub)
